...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-8 w-full z-40 border-b border-border bg-background/80 backdrop-blur-md">
          ...
          <div className="md:hidden py-4 border-t border-border animate-fade-in-up max-h-[calc(100vh-6rem)] overflow-y-auto overscroll-contain">
            <div className="flex flex-col gap-2">
              ...
              <div className="border-b border-border pb-2 mb-2">
                ...
                {resourcesMenu.map((item) => {
                  const cls = "px-4 py-2 rounded-lg text-sm font-medium transition-colors text-foreground/70 hover:bg-muted flex flex-col gap-1";
                  const inner = (
                    <>
                      <span>{item.name}</span>
                      <span className="text-xs text-muted-foreground">{item.description}</span>
                  ...
                  return item.external ? (
                    <a key={item.path} href={item.path} target="_blank" rel="noopener noreferrer" onClick={() => setMobileMenuOpen(false)} className={cls}>{inner}</a>
                  ) : (
